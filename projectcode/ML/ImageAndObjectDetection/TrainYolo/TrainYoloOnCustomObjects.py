...
DATA_YAML   = "/app/mediaFiles/images/YoloTrainingImages/yolotrainingReadyImages/YoloIterations/iterationI/trainingconfigurations/config.yml"
...
def pick_model_path(arg_model: str, weights_dir: Path) -> str:
    if arg_model:
        return _p(arg_model)
    cand = Path(weights_dir) / DEFAULT_MODEL
    return str(cand) if cand.exists() else DEFAULT_MODEL  # Ultralytics will download if needed
